...
from Book
         INNER join Publisher on Book.Pub_Id = Publisher.Pub_Id
where Book.Price > (select AVG(Price)
                      FROM Book);
...
from Member
         left join Book_Invoice on Member.CardNo = Book_Invoice.CardNo
...
where Book.Book_Id in ( select Book_Id
                        ...
                        where(
                           ...
                           or
                            (Book_Invoice.Return_Date is null and Book_Invoice.Due_Date < curdate() and DATEDIFF(curdate(), Book_Invoice.Due_Date) > 7)))
